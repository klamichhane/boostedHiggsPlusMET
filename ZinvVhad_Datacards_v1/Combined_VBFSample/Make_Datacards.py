...
for j in sig:
    for i in h:
        ...
        os.system("python Datacard_NoVBF.py {0} {1} {2}".format(i, j, year))
        print " "
...
if len(sig)==2:
    for i in h:
        ...
        os.system("combineCards.py {2}_{1}/{2}_Card_MG{0}_bin*{1}.txt {3}_{1}/{3}_Card_MG{0}_bin*_{1}.txt > Combo_{3}_{1}/Combined_VBFnVBFfail_{3}_{0}_{1}.txt".format(i,year,sig[0],sig[1]))

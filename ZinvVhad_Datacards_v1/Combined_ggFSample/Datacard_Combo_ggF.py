...
if sample=="VBFG" or sample=="ggFG": sam="ggFG"
if sample=="VBFRad" or sample=="ggFRad": sam="ggFRad"
if sample=="VBFWp" or sample=="ggFWp": sam="ggFWp"
...
for j in xrange(1,nbins+1):
    ...
    with open(fname,'w') as f :
        ...
        f.write("bkgPdfUnc lnN - {1} - - {1} -\n".          format(j,histo[8].GetBinContent(j),cat1))
        f.write("bkgScaleUnc lnN - {1} - - {1} -\n".        format(j,histo[9].GetBinContent(j),cat1))
        f.write("rareUnc_{0} lnN - - 1.2 - - 1.2\n".        format(j))
        f.write("sigUnc_{0} lnN 1.2 - - 1.2 - -\n".         format(j))
        f.write("SigSR_stat_{2}_{0} lnN {1} - - - - -\n".   format(j,1+(histo[0].GetBinError(j)/histo[0].GetBinContent(j)),cat1))
        f.write("BkgSR_stat_{2}_{0} lnN - {1} - - - -\n".   format(j,1+(histo[1].GetBinError(j)/histo[1].GetBinContent(j)),cat1))
        f.write("RareSR_stat_{2}_{0} lnN - - {1} - - -\n".  format(j,1+(histo[2].GetBinError(j)/histo[2].GetBinContent(j)),cat1))
        f.write("SigSB_stat_{2}_{0} lnN - - - {1} - -\n".   format(j,1+(histo[3].GetBinError(j)/histo[3].GetBinContent(j)),cat1))
        f.write("BkgSB_stat_{2}_{0} lnN - - - - {1} -\n".   format(j,1+(histo[4].GetBinError(j)/histo[4].GetBinContent(j)),cat1))
        f.write("RareSB_stat_{2}_{0} lnN - - - - - {1}\n".  format(j,1+(histo[5].GetBinError(j)/histo[5].GetBinContent(j)),cat1))
        f.close()    

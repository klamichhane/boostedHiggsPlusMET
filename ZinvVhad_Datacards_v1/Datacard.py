...
elif sample=="ggFG" or sample=="ggFWp" or sample=="ggFRad":
...
#if "ggF" in sample: startbin = 4
#else: startbin = 1
startbin = 1

for j in xrange(startbin,nbins+1):
    fname=inputfoldername+"/"+sample+"_"+year+"/"+sample+"_Card_M{0}_bin{1}_{2}.txt".format(mass,int(histo[0].GetBinLowEdge(j)),year)
    sRcat = "S{}".format(j)
    cRcat = "C{}".format(j)
    with open(fname,'w') as f :
        ...
        (histo[0].GetBinContent(j)*0.001*scale), # 0.001 scaling Xsec to 1fb  
        ...
        f.write("bkgPdfUnc lnN - {1} - - {1} -\n".          format(j,histo[8].GetBinContent(j)))
        f.write("bkgScaleUnc lnN - {1} - - {1} -\n".        format(j,histo[9].GetBinContent(j)))
